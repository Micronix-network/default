 ...
 * File:        chromatable.js
 * Version:     1.3.0
 * CVS:         $Id: jquery.chromatable.js,v 1.2 2014/01/07 09:15:06 riboldi01 Exp $
...
(function($) {
	...
	$.fn.chromatable = function(options) {
		...
		return this
				.each(function() {
					...
					// Add dimentsions from user or default parameters to the
					// DOM elements
					...
					// clone an exact copy of the scrolling table and add to DOM
					// before the original table
					// replace old class with new to differentiate between the
					// two
					$(this).before(
					...
					// remove all children within the cloned table after the
					// thead element
					...
					$(this)
							.each(
									function($this) {
										...
										// if the width is auto, we need to
										// remove padding-right on scrolling
										// container
										...
										if (options.scrolling == "no") {
											...
											// this is dependant on the jQuery
											// resizable UI plugin
											$("#" + $uniqueID).resizable({
												handles : 's'
					...
					// check to see if the width is set to auto, if not, we
					// don't need to call the resizer function
					if (options.width == "100%" || "auto") {

						// call the resizer function whenever the table width
						// has been adjusted
	...
	// private function to temporarily hide the header when the browser is
	// resized
