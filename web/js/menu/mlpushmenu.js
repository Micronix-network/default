			...
			if( subLevel ) {
				...
				for( var i = 0, len = this.levels.length; i < len; ++i ) {
					...
					if( levelEl != subLevel && !$(levelEl).hasClass('mp-level-open' ) ) {
						this._setTransform( '-100%,' + -1*levelFactor + 'px', levelEl );
					}
				}
			}
			...
			if(val!=''){
				...
				if(this.support){
					for (index = 0; index < values.length; ++index) {
						command+=' translate3d('+values[index]+',0,0)';
					}
				}else{
					for (index = 0; index < values.length; ++index) {
						command+=' translateX('+values[index]+')';
					}
				}
			}
			el.style.WebkitTransform = command;
			el.style.MozTransform = command;
			el.style.transform = command;
			el.style.msTransform = command;
		},
